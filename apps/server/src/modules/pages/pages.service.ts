...
/**
 * Check if an organisation has E2EE enabled.
 * When encrypted, the server skips plaintext processing (search indexing, text extraction).
 */
async function isOrgEncrypted(orgId: string): Promise<boolean> {
  const org = await prisma.organization.findUnique({
    where: { id: orgId },
    select: { isEncrypted: true },
  });
  return org?.isEncrypted ?? false;
}
...
export async function updatePage(
  ...
  // Skip plaintext processing for E2EE workspaces — content is encrypted client-side
  const encrypted = await isOrgEncrypted(orgId);

  // Block public sharing for encrypted workspaces — content is E2EE and cannot be viewed publicly
  if (input.isPublic === true && encrypted) {
    throw new Error('PUBLIC_SHARING_DISABLED_ENCRYPTED');
  }

  // Auto-generate slug when making page public without providing one
  let autoGeneratedSlug: string | undefined;
  if (input.isPublic === true && !page.publicSlug && input.publicSlug === undefined) {
    autoGeneratedSlug = await generatePublicSlug(page.title);
  }
  const plainContent =
    !encrypted && input.htmlContent !== undefined ? htmlToText(input.htmlContent ?? '') : undefined;
  ...
  const updated = await prisma.page.update({
    ...
    data: {
      ...
      ...(autoGeneratedSlug !== undefined && { publicSlug: autoGeneratedSlug }),
  ...
  // Skip search indexing for E2EE workspaces — encrypted content cannot be searched server-side
  if (!encrypted) {
    indexPage({
      id: updated.id,
      title: updated.title,
      plainContent: plainContent ?? updated.plainContent ?? '',
      orgId: orgId,
...
export async function restorePage(orgId: string, pageId: string): Promise<Page> {
  ...
  indexPage({
    ...
    plainContent: restored.plainContent ?? '',
    orgId: orgId,
